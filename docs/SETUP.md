...
5. **Mobile config (`@react-native-firebase`)**: the app uses the native
   Firebase SDK (handles phone auth/SMS without reCAPTCHA). No JS config object
   is needed — instead add the native config file:
   - Firebase console → ⚙️ Project settings → **Your apps** → **Add app →
     Android**.
   - Package name: **`com.babyphone`** (must match the app id).
   - Download **`google-services.json`** → place it in
     `mobile/android/app/google-services.json` (gitignored).
   - Gradle wiring (in the project generated by `react-native init`):
     - `android/build.gradle` → `dependencies { classpath
       'com.google.gms:google-services:4.4.2' }`
     - `android/app/build.gradle` → add at the top:
       `apply plugin: 'com.google.gms.google-services'`
   - Test phone numbers configured in the console (e.g. `+2250700000000` /
     `123456`) log in without a real SMS.
